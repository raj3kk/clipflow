...
export async function GET(req: Request) {
  ...
  // v2 (phone) submissions bhi — taaki user dekh sake kya submit hua
  const { data: v2subs } = await sb
    .from("v2_submissions")
    .select("id, campaign_slug, reel_url, whop_status, submitted_at, created_at")
    .eq("user_id", userId)
    .order("submitted_at", { ascending: false })
    .limit(50);
  return NextResponse.json({ submissions: data, v2_submissions: v2subs ?? [], configured: true });
}
